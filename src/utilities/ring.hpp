...
class ring {
  public:
#if nsrs_RING_SPAN_LITE_EXTENSION
	typedef ring_span<
		typename Container::value_type, default_popper<typename Container::value_type>, CapacityIsPowerOf2>
		RingSpan;
#else
	...
	explicit ring(size_type capacity) :
		cont(capacity), rs(cont.begin(), cont.end()) {}
	...
	nsrs_REQUIRES_0(std::is_copy_assignable<value_type>::value) void push_back(value_type const &value) noexcept(std::is_nothrow_copy_assignable<value_type>::value)
#else
...
#if nsrs_CPP11_OR_GREATER
	nsrs_REQUIRES_0(std::is_move_assignable<value_type>::value) void push_back(value_type &&value) noexcept(std::is_nothrow_move_assignable<value_type>::value) {
...
#if nsrs_CPP11_OR_GREATER
	nsrs_REQUIRES_0(std::is_copy_assignable<value_type>::value) void push_front(value_type const &value) noexcept((std::is_nothrow_copy_assignable<value_type>::value))
#else
...
#if nsrs_CPP11_OR_GREATER
	nsrs_REQUIRES_0(std::is_move_assignable<value_type>::value) void push_front(value_type &&value) noexcept((std::is_nothrow_move_assignable<value_type>::value)) {
