...
#pragma once
#pragma warning(disable : 4267 4996 4244 4848)
...
#define nsrs_DISABLE_MSVC_WARNINGS(codes) __pragma(warning(push)) __pragma(warning(disable \
																				   : codes))
...
#else
template <typename T>
T const &move(T const &t) { return t; }
#endif
...
//
template <
	class T, class Popper = default_popper<T>
#if nsrs_RING_SPAN_LITE_EXTENSION
	,
...
#endif
{
  public:
	...
	ring_span(
		ContiguousIterator begin, ContiguousIterator end, Popper popper = Popper()) nsrs_noexcept
#if nsrs_CONFIG_POPPER_EMPTY_BASE_CLASS
	...
	template <class ContiguousIterator>
	ring_span(
		ContiguousIterator begin, ContiguousIterator end, ContiguousIterator first, size_type size, Popper popper = Popper()) nsrs_noexcept
#if nsrs_CONFIG_POPPER_EMPTY_BASE_CLASS
		: Popper(std11::move(popper)),
		  m_data(&*begin),
		  m_size(size),
...
#if nsrs_CPP11_OR_GREATER
	nsrs_REQUIRES_0(std::is_copy_assignable<T>::value) void push_back(value_type const &value) noexcept(std::is_nothrow_copy_assignable<T>::value)
#else
...
#if nsrs_CPP11_OR_GREATER
	nsrs_REQUIRES_0(std::is_move_assignable<T>::value) void push_back(value_type &&value) noexcept(std::is_nothrow_move_assignable<T>::value) {
		if (full())
			increment_front_and_back_();
		else
			increment_back_();
...
#if nsrs_CPP11_OR_GREATER
	nsrs_REQUIRES_0(std::is_copy_assignable<T>::value) void push_front(T const &value) noexcept((std::is_nothrow_copy_assignable<T>::value))
#else
...
#if nsrs_CPP11_OR_GREATER
	nsrs_REQUIRES_0(std::is_move_assignable<T>::value) void push_front(T &&value) noexcept((std::is_nothrow_move_assignable<T>::value)) {
		if (full())
			decrement_front_and_back_();
		else
			decrement_front_();
